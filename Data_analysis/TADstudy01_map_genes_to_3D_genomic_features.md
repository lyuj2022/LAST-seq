...
##### Call TADS (follow the HiCExplorer mannual)

```shell
#convert hic to cool for resolution of 100K
hicConvertFormat -m HAP1.hic --inputFormat hic --outputFormat cool -o wt_matrix.cool --resolutions 100000
...
#use R scripts ("extract_TADbound_coor_0.05.R") to ajust coordinates of TAD boundary.
# the resulant files are named "TADbound_0.05.bed" 
#see Data file for the resultant data
```

##### Map genes to TADs, TAD boundaries and A/B compartments by bedops
...
#sort cordinates for TADs, TADboundaries, AB compartments
sort-bed gene.coor.bed > gene.coor.sorted.bed
sort-bed TADbound_0.05.bed > TADbound_0.05.sorted.bed
sort-bed ABcompartments.bed > ABcompartments.sort.bed

#map genes to TADs, TADboundaries, AB compartments
bedmap --fraction-ref 1 --echo --echo-map-id-uniq --delim '\t' gene.coor.sorted.bed wt_min300k_max1M_step100k_thres0.05_delta0.01_fdr_domains.bed > wt_gene_TADs.bed
