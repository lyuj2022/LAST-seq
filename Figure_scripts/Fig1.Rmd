...
```{r mapping rate of SMART-seq Figs4f}
library(tidyverse)
CEL_exon_intron <- read.delim("./SMARTv4.readspercell.txt",header = T)
CEL_exon_intron <- CEL_exon_intron %>%
  filter(RG  != 'bad')
...
ggsave(filename = "SMARTseq_Mapping.pdf",plot = p,width=45,height = 45,units = "mm", dpi = 300)
```

```{r LAST-seq vs SMART-seq reads SF 4g}
# load last-seq dataset
last_read1 <- as.data.frame(table_1000K_reads)
last_read1$ID <- rownames(last_read1)
...
# load smart-seq dataset
AllCounts <- readRDS("SMARTv4.dgecounts.rds")
downsample_1000K_ex <- AllCounts$readcount$ex$downsampling$downsampled_1000000
table_1000K_ex <- as.matrix(downsample_1000K_ex)
table_1000K_ex <- table_1000K_ex[!grepl("^ERCC-",rownames(table_1000K_ex)),]

SMARTfull<- table_1000K_ex
...
p <-last_smart_readsR%>% ggplot(aes(x=meanreadssmart, y=lastmeadreads))+
  geom_point(size=0.1)+
  theme_man+
  ...
  annotation_logticks(sides = "bl", size=0.15, short = unit(0.05, "cm"), mid = unit(0.1, "cm"), long = unit(0.15, "cm")) +
  annotate(geom="text",x=5, y=30000, label="PCC=0.644",size=2)+
  xlab("SMART-seq read numbers")+  
  ylab("LAST-seq read numbers")
...
```{r LAST-seq vs SMART-seq ERCC detection SF 4i}
...
megsmart_last <- megsmart_last %>%
  filter(copy_number > 5)
...
p <- megsmart_last%>%ggplot(aes(x=copy_number,y=detection_rate,color=type))+
  ...
  xlab("spike-ins copy#")+
  ylab("detection rate of ERCC spike-in")+
